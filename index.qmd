...
df <- data %>%
  ...
  mutate(Annual2023Income = case_when(
         ...
         Annual2023Income == Income2023M2Y & Annual2023Income > 999 & Annual2023Income < 10000 ~ Annual2023Income*12,
...
```{r overview, dev = "png", dev.args=list(bg="#9bc0c6")}
...
df %>%
  ...
  labs(fill = "Race", title = "Distribution of Respondents by Race") +
  theme_minimal() +
  theme(
...
ggplot() +
  geom_histogram(data = df %>% filter(Annual2023Income < 200000), aes(x = Annual2023Income, fill = palette_cpal_main[1]), binwidth = 5000, boundary = 0) +
  scale_x_continuous(
    labels = scales::dollar_format(),
    limits = c(0, 225000)
  ) +
  labs(title = "Income Distribution of Respondents (Under $200,000)") +
  theme_minimal() +
...
df %>%
  ggplot(aes(x = age_yr, fill = palette_cpal_main[1])) +
  geom_histogram(data = df, aes(x = age_yr, fill = palette_cpal_main[1]), binwidth = 2.5) +
...
df %>%
  ...
  labs(title = "Median Income of Respondents by Race") +
  theme_minimal() +
  theme(legend.position = "none",
        axis.title = element_blank(),
        plot.background = element_rect(fill = "transparent", colour = NA),
...
leaflet(zctas_merged) %>%
  ...
  addLegend(pal = colorNumeric("YlGn", zctas_merged$n, n = 5), values = zctas_merged$n, title = "Count")
...
# 
#     # generate leaflet bounding box dependent on geography type selected
#     bbox <- filter(cpalmetrics, NAME == "Dallas County, Texas" & YEAR == input$year) %>%
#       st_bbox(.) %>%
#       as.vector()
# 
#     # select leaflet options for displayed map
#     leaflet() %>%
...
#   })
# 
# palbins <- unique(BAMMtools::getJenksBreaks(mapData[[mappingGroup()]], k = 6))
# 
#     pal <- colorBin(palette = "BuPu",
#                     bins = palbins,
#                     domain = mapData[[mappingGroup()]],
#                     na.color = "#595959")
# 
#     leafletProxy("map") %>%
#       clearShapes() %>%
...
#                   fillColor = ~pal(mapData[[mappingGroup()]]),
#                   color = "#595959",
#                   weight = 0.3,
#                   
# addPolygons(data = mapData,
...
# education_race <- df %>%
#   group_by(CPAL.DEMO.007.2, Education_Category) %>%
#   tally(name = "count_education_race") %>%
#   mutate(total_race = sum(count_education_race, na.rm = TRUE)) %>%
#   mutate(percent = count_education_race / total_race) %>%
#   ggplot(aes(x = percent, y = Education_Category, color = CPAL.DEMO.007.2)) +
...
# 
# cpal_plot(zeroGrob(),
#           cpal_title(string = "Percent of Race at Education Level",
#                      width = 75),
#           education_race,
#           cpal_logo_text(), 
#           ncol = 1,
...
# 
# income_race_education <- df %>%
#   group_by(CPAL.DEMO.007.2, Education_Category) %>%
#   summarise(med_income = median(Annual2023Income, na.rm = TRUE)) %>%
#   ggplot(aes(x = CPAL.DEMO.007.2, y = med_income, color = CPAL.DEMO.007.2)) +
#   geom_segment(aes(xend = CPAL.DEMO.007.2, yend = 0), size = 0.5) +
#   geom_point(size = 4, alpha = 0.8) +
#   facet_wrap(~ Education_Category) +
#   labs(x = "", y = "") +
#   theme_minimal() +
#   theme(legend.position = "none",
...
# 
# cpal_plot(zeroGrob(),
#           cpal_title(string = "Median Income by Education Level and Race",
#                      width = 75),
#           income_race_education,
#           cpal_logo_text(), 
#           ncol = 1,
#           heights = c(1, 3, 30, 3, 2, 2))
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.JOB.015)))/nrow(df), accuracy = 0.01)`
:::
...
df %>% 
  ...
  geom_text(aes(label = ifelse(percentage > 0.15, scales::percent(percentage, scale = 100, accuracy = .1), "")), 
...
**Rate of response (all respondents):** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.BEN.001.b)))/nrow(df), accuracy = 0.01)`

**Rate of response (receives food benefits):** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.BEN.001.b)))/nrow(df %>% filter(!is.na(CPAL.BEN.001.b))), accuracy = 0.01)`
:::
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.JOB.008)))/nrow(df), accuracy = 0.01)`
:::
...
df %>%
  ...
  geom_bar(width = 1, stat = "identity") +
  coord_polar(theta = "y", start = 0) +
  geom_text(aes(label = ifelse(prop > 0.05, scales::percent(prop * 100, scale = 1, accuracy = 1), "")), 
...
**The Dallas County median income in 2021 was `r scales::dollar(dallas_county_acs %>% pull(B19013_001))`, so this survey sample is poorer than Dallas County as a whole.**
...
cpal_plot(violin,
          cpal_note(paste0(nrow(df %>% filter(Annual2023Income > 199999.99)), " respondents reporting income above $200,000 not visualized. Maximum reported income is ", scales::dollar(max(df$Annual2023Income, na.rm = T)))),
          heights = (c(9,1))
)
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(POVERTYLine2023)))/nrow(df), accuracy = 0.01)`
:::

::: g-col-7

**The Dallas County poverty rate in 2021 was `r scales::percent(dallas_county_acs %>% pull(povertyRate))`, so this survey sample is poorer than Dallas County as a whole.**
...
df %>%
  ...
  geom_text(aes(label = scales::percent(prop), y = prop, size = 2), color = "white", position = position_stack(vjust = 0.5)) +
  geom_text(aes(label = POVERTYLine2023, y = prop, x = 0.6, size = 2), position = position_stack(vjust = 0.5)) +
  labs(title = NULL,
       x = NULL,
       y = NULL) +
  theme_minimal() +
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.EHQ.001)))/nrow(df), accuracy = 0.01)`
:::
...
**`r scales::percent(nrow(df %>% filter(is.numeric(CPAL.EHQ.001), CPAL.EHQ.001 < 2))/nrow(df %>% filter(is.numeric(CPAL.EHQ.001))), accuracy = 1)` of respondents report a decrease in monthly income, while only `r scales::percent(nrow(df %>% filter(is.numeric(CPAL.EHQ.001), CPAL.EHQ.001 > 2))/nrow(df %>% filter(is.numeric(CPAL.EHQ.001))), accuracy = 1)` report an increase.**
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.EHQ.002)))/nrow(df), accuracy = 0.01)`
:::
...
**`r scales::percent(nrow(df %>% filter(is.numeric(CPAL.EHQ.002), CPAL.EHQ.002 > 0))/nrow(df), accuracy = 1)` of respondents report at least some family financial problems.**
...
df %>%
  ...
  geom_text(aes(label = ifelse(prop > 0.1, scales::percent(prop, scale = 100), "")), 
            ...
            size = 8, 
            colour = "white") +
  geom_text(aes(label = ifelse(prop <= 0.1, scales::percent(prop, scale = 100, accuracy = .1), "")), 
        position = position_dodge(width=0.9), 
        hjust = -0.1,
        size = 8, 
        colour = "darkgrey") +
...
df %>%
  ...
  geom_text(aes(label = ifelse(financial_problems == "yes", scales::percent(round(prop, digits = 2), scale = 100), "")), 
...
**`r scales::percent(nrow(df %>% filter(is.numeric(CPAL.EHQ.002), CPAL.EHQ.002 > 0, !is.na(POVERTYLine2023), as.numeric(POVERTYLine2023) == 1))/nrow(df %>% filter(is.numeric(CPAL.EHQ.002), !is.na(POVERTYLine2023), as.numeric(POVERTYLine2023) == 1)), accuracy = 1)` of those under the poverty line report family financial problems, as do `r scales::percent(nrow(df %>% filter(is.numeric(CPAL.EHQ.002), CPAL.EHQ.002 > 0, !is.na(POVERTYLine2023), as.numeric(POVERTYLine2023) == 2))/nrow(df %>% filter(is.numeric(CPAL.EHQ.002), !is.na(POVERTYLine2023), as.numeric(POVERTYLine2023) == 2)), accuracy = 1)` of those above the poverty line.**
...
df %>%
  ...
  geom_bar(stat = "identity", position = "stack", width = 0.9) +
  geom_text(aes(label = ifelse(financial_problems == "yes", scales::percent(prop, scale = 100), "")), 
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.FSTR.001)))/nrow(df), accuracy = 0.01)`
:::
...
**`r scales::percent(nrow(df %>% filter(is.numeric(CPAL.FSTR.001), CPAL.FSTR.001 > 0))/nrow(df), accuracy = 1)` of respondents say paying for basics is difficult.**
...
**`r scales::percent(1 - nrow(df %>% filter(is.numeric(CPAL.FSTR.001), CPAL.FSTR.001 > 0, POVERTYLine2023 == 1))/nrow(df %>% filter(is.numeric(CPAL.FSTR.001), CPAL.FSTR.001 > 0)), accuracy = 1)` of respondents reporting difficulty paying for basics are *over* the poverty line.** 
...
**`r scales::percent(nrow(df %>% filter(is.numeric(CPAL.FSTR.001), CPAL.FSTR.001 > 0, !is.na(CPAL.CBCL.015)))/nrow(df %>% filter(!is.na(CPAL.CBCL.015))), accuracy = 1)` of respondents with at least one child 6 years old or younger report difficulty paying for basics.**
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.FTAX.001)))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response (if applicable):** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.HEALTH.003.2)))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.POLICY.035)))/nrow(df), accuracy = 0.01)`
:::
...
df %>%
  ...
  mutate(`Income Bracket` = case_when(
    Annual2023Income < 30000 ~ "$30,000 or less",
    Annual2023Income >= 30000 & Annual2023Income <= 49999 ~ "$30,000 to $49,999",
    Annual2023Income > 49999 & Annual2023Income <= 69999 ~ "$50,000 to $69,999",
    Annual2023Income > 69999 & Annual2023Income <= 89999 ~ "$70,000 to $89,999",
    Annual2023Income > 89999 ~ "$90,000 and above"
  )) %>%
  ...
  geom_text(aes(label = ifelse(childcare == "Not looking", scales::percent(prop, scale = 100), "")),
        position = position_stack(), vjust = 0, hjust = -0.1, size = 5, colour = "darkgrey") +
  geom_bar(stat = "identity", position = "stack", width = 0.9) +
  coord_flip() +
...
**Rate of response (if applicable):** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.POLICY.037), CPAL.POLICY.037 != -99))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.HEALTH.002)))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response (if applicable):** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.HEALTH.004.2), CPAL.HEALTH.004.2 != -99))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.DEBT.007)))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.HOME.005)))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.HOME.006)))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response (renters):** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.MH.009)))/nrow(df %>% filter(CPAL.DEBT.007 == 1)), accuracy = 0.01)`

**Rate of response (total):** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.MH.009)))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response (homeowners):** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.MH.010)))/nrow(df %>% filter(CPAL.DEBT.007 == 2)), accuracy = 0.01)`

**Rate of response (total):** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.MH.010)))/nrow(df), accuracy = 0.01)`
:::
...
**Over `r scales::percent(nrow(df %>% filter(CPAL.MH.010 == 1, !is.na(CPAL.MH.010)))/nrow(df %>% filter(!is.na(CPAL.MH.010))), accuracy = 1)` of homeowners report being foreclosed on in the past 30 days.**
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(rowSums(is.na(.[grepl("^CPAL.DEMO.011", names(.))])) < ncol(.)))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(rowSums(is.na(.[grepl("^CPAL.DALLASCORE.003_", names(.))])) < ncol(.)))/nrow(df), accuracy = 0.01)`
:::

::: g-col-7

<!-- **`r scales::percent(nrow(df %>% filter(CPAL.MH.010 == 1, !is.na(CPAL.MH.010)))/nrow(df %>% filter(!is.na(CPAL.MH.010))), accuracy = 1)` of respondents with at least one child aged 6 or younger in the home report a household member spending at least one night in jail or prison in the past 3 months.** -->
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(!is.na(CPAL.CESD.001.b)))/nrow(df), accuracy = 0.01)`
:::
...
**Rate of response:** `r scales::percent(nrow(df %>% filter(rowSums(is.na(.[grepl("^CPAL.DALLASSCORE.002_", names(.))])) < ncol(.)))/nrow(df), accuracy = 0.01)`
:::
...
df %>% 
  ...
  geom_text(aes(label = ifelse(percentage > 0.15, scales::percent(percentage, scale = 100, accuracy = 1), "")), 
          position = position_dodge(width=0.9), hjust = 1.1, size = 7, colour = "white") +
  geom_text(aes(label = ifelse(percentage <= 0.15, scales::percent(percentage, scale = 100, accuracy = 1), "")), 
        position = position_dodge(width=0.9), hjust = -0.1, size = 7, colour = "darkgrey") +
scale_x_continuous(labels = scales::percent_format(scale = 100), limits = c(0,1)) +
  theme_minimal() +
  theme(
    legend.position = "none",
